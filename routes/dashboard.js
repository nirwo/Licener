...
router.get('/', ensureAuthenticated, async (req, res) => {
  try {
    ...
    const subscriptions = await Subscription.find({ user: req.user._id }).sort({ created: -1 }).limit(5);
    ...
    const recentSubscriptions = await Subscription.find({ user: req.user._id }).sort({ created: -1 }).limit(5);

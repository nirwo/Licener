...
router.get('/dashboard', ensureAuthenticated, async (req, res) => {
  try {
    ...
    allLicensesInDb.forEach((license, index) => {
      console.log(
        `License ${index + 1} in DB: ID=${license._id}, Owner=${license.owner}, Product=${license.product || license.name}`
      );
    });
    ...
    if (userId) {
      allUserLicenses = allLicensesInDb.filter(license => {
        ...
        console.log(
          `License ${license._id} - Owner: ${licenseOwner}, User: ${userId}, Match: ${isMatch}`
        );
        return isMatch;
      });
    }
    ...
    if (allUserLicenses.length > 0) {
      ...
      allUserLicenses.forEach((license, index) => {
        console.log(
          `Dashboard License ${index + 1}: ID=${license._id}, Name=${license.name || license.product}`
        );
      });
    } else {
    ...
    console.log(
      `Dashboard - Found ${userSystems.length} systems managed by this user out of ${allUserSystems.length} total systems.`);

    // Use all systems for display if none found for this user and not demo
    const allUserSystemsForDisplay =
      userSystems.length > 0 ? userSystems : userId !== 'u39dks82bn' ? allUserSystems : [];
    ...
    allUserLicenses.forEach(lic => {
      if (lic.expiryDate) {
        console.log(
          `License ${lic._id} expiry: ${lic.expiryDate}, in 30-day window: ${new Date(lic.expiryDate) <= thirtyDaysFromNow}`
        );
      }
    });
    ...
    console.log('License utilization data:', {
      ...
      utilizationData: utilizationLicenses.map(
        l =>
    ...
    for (let i = 0; i < 6; i++) {
      ...
      if (i === 0)
        expiryTimelineData.colors.push('#e74a3b'); // Red
      else if (i === 1)
        expiryTimelineData.colors.push('#f6c23e'); // Orange/Yellow
    ...
    chartData = {
      expiryTimeline: {
        labels:
          expiryTimelineData.labels.length > 0
            ? expiryTimelineData.labels
            : ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun'],
        data: expiryTimelineData.data.length > 0 ? expiryTimelineData.data : [0, 0, 0, 0, 0, 0],
        colors:
          expiryTimelineData.colors.length > 0
            ? expiryTimelineData.colors
            : ['#e74a3b', '#f6c23e', '#4e73df', '#4e73df', '#4e73df', '#4e73df'],
      },
      vendors: {
      ...
      costs: {
        ...
        data:
          Object.values(vendorCosts).length > 0
            ? Object.values(vendorCosts).map(cost => cost.toFixed(2))
            : [0],
      },
      colors: chartColors,
    };
    ...
    const expiringDemoLicenses = demoLicenses.filter(
      lic => moment(lic.expiryDate).diff(moment(), 'days') <= 30
    );

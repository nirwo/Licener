...
exports.searchVendors = async (req, res) => {
  ...
  try {
    logger.info(`User ${req.user.email} is searching for vendors with term "${searchTerm}" using source "${source}"`);
    ...
    if (!results || results.length === 0) {
      req.flash('warning', 'No vendors found for your search term. Try a different term or source.');
      return res.redirect('/vendors/research');
    }
...
exports.addVendorFromResults = async (req, res) => {
  ...
  try {
    ...
    logger.info(`User ${req.user.email} added new vendor "${newVendor.name}" from research results`);

...
describe('License Model', function() {
  ...
  it('should identify expired and expiring licenses', async function() {
    try {
      ...
      await Promise.all([
        new License({...testLicense, name: 'Expired License', expiryDate: pastDate, status: 'expired'}).save(),
        new License({...testLicense, name: 'Expiring Soon License', expiryDate: futureDate}).save(),
